...
import { textSize, listType } from './theme.js';
...
function Markdown({ children }) {
    const components = {
        p: ({ children }) => <Paragraph fontSize={6}>{children}</Paragraph>,
        ...
        ul: ({ children }) => <ul className={`list-inside ${listType.bulleted} ${textSize[6]} text-black bg-transparent text-left`}>{children}</ul>,
        ol: ({ children }) => <ol className={`list-inside ${listType.numbered} ${textSize[6]} text-black bg-transparent text-left`}>{children}</ol>,
        ...
        a: ({ children, href }) => <Link href={href} fontSize={6}>{children}</Link>
    };
